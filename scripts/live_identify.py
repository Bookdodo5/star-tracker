...
def calibrate_fov(lib, bgr, fov_min, fov_max, step=1.01, morph=1):
    """Sweeps FOV geometrically over one frame until a solve; returns (fov, attitude) or (None, None).

    TETRA tolerates ~1% FOV error, so a 1% multiplicative step guarantees a grid point within
    tolerance of the true FOV. The verifier's mean-residual gate makes the first solve trustworthy,
    so the FOV that first solves is the real one. Use this once to lock a fixed camera/screen FOV.
    """
    fov = fov_min
    while fov <= fov_max:
        att = solve(lib, bgr, fov, morph)
        if att:
            return fov, att
        fov *= step
    return None, None
...
def main():
    ...
    parser.add_argument("--fov-search", action="store_true",
                        help="auto-calibrate FOV: sweep --fov-min..--fov-max until the first solve, then lock it. "
                             "Use when the true FOV is unknown (fixed camera/screen).")
    parser.add_argument("--fov-min", type=float, default=8.0, help="lowest FOV to try with --fov-search")
    parser.add_argument("--fov-max", type=float, default=40.0, help="highest FOV to try with --fov-search")
    ...
    if args.fov_search:
        print(f"[live] FOV search {args.fov_min}..{args.fov_max} (~1% steps); will lock on first solve")
    ...
    try:
        while True:
            ...
            if args.scale != 1.0:
                bgr = cv2.resize(bgr, None, fx=args.scale, fy=args.scale, interpolation=cv2.INTER_AREA)
            if not locked:  # sweep this frame for the FOV that solves, then reuse it
                found, att = calibrate_fov(lib, bgr, args.fov_min, args.fov_max, morph=args.morph)
                if found:
                    args.fov, locked = found, True
                    print(f"[live] locked FOV = {found:.3f} deg")
                else:
                    print(f"frame {frame_i:4d} | no solve across FOV range; retrying next frame")
                    continue
            else:
