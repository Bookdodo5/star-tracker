...
/** Per-grow-attempt branch cap; reset before each grow() call so later seeds get a fair budget. */
#define PYRAMID_GROW_BRANCH_CAP 400
/** Exit as soon as a match this good is verified; avoids exhausting all seed pairs needlessly. */
#define PYRAMID_EARLY_EXIT_SCORE 70000
#define PYRAMID_SEED_TOL_CODE 328u
#define PYRAMID_GROW_TOL_CODE 200u
...
typedef struct {
    ...
    uint16_t branches;
    uint32_t verify_us;
    /* Number of observed stars to verify against (the full detected set), which can
       exceed the brightest query stars used to seed and grow patterns. */
 ...
 */
static int32_t pair_score(const PairCandidate *c) {
    /* Signed: each vote subtracts 256 from the penalty, so the correct well-voted
       pair scores negative (best) while unvoted incorrect pairs score positive (worst).
       The old uint32 version underflowed, making correct pairs sort last. */
...
static void insert_pair_candidate(
    ...
    int32_t candidate_score = pair_score(&candidate);
    uint8_t insert_position = *nearest_pair_count;
    if (insert_position < PYRAMID_MAX_SEED_CANDIDATES) {
        ++(*nearest_pair_count);
    } else {
        if (candidate_score >= pair_score(&nearest_pair_candidates[insert_position - 1])) {
            return;
        }
        insert_position = PYRAMID_MAX_SEED_CANDIDATES - 1;
    }
    while (insert_position > 0 && pair_score(&nearest_pair_candidates[insert_position - 1]) > candidate_score) {
    ...
    for (uint8_t first_pattern_index = 0; first_pattern_index < PYRAMID_SIZE - 1; ++first_pattern_index) {
        for (uint8_t second_pattern_index = first_pattern_index + 1; second_pattern_index < PYRAMID_SIZE; ++second_pattern_index) {
            ...
            if (solve_attitude_triad(pair_hr_ids, pair_obs_ids, 2, observed_stars, &rotation) &&
                ...
                /* Verify inliers across the full observed set, not just the query stars
                   used to grow the pattern: real images carry more true catalog stars
                   than the brightest-query pool, and a wider inlier count rescues correct
                   solves and outscores false positives. */
                bool verified_ok = verify_attitude(&rotation, observed_stars, state->verify_count, &verified);
                state->verify_us += elapsed_us(verify_start, clock());
                if (verified_ok && (!state->best.success || verified.score > state->best.score)) {
                    state->best = verified;
                }
            }
        }
...
static void grow(
    ...
    if (++state->branches > PYRAMID_GROW_BRANCH_CAP) {
        return;
    }
    if (size == PYRAMID_SIZE) {
        evaluate_pattern(observed_stars, obs_ids, hr_ids, state);
        return;
    }
    ...
    for (uint8_t candidate_index = 0; candidate_index < count; ++candidate_index) {
        ...
        /* Stop this attempt when over budget or a high-confidence match was already found. */
        if (state->branches > PYRAMID_GROW_BRANCH_CAP ||
            state->best.score >= PYRAMID_EARLY_EXIT_SCORE) {
            return;
        }
    }
}
...
bool identify_pyramid(const ObservedStar *observed_stars, uint8_t observed_star_count, MatchResult *result) {
    ...
    /* Seed/grow from the brightest query stars, but verify against every detected star. */
    state.verify_count = observed_star_count;
    for (uint8_t first_seed_observed_index = 0; first_seed_observed_index < query_star_count - 1; ++first_seed_observed_index) {
        for (uint8_t second_seed_observed_index = first_seed_observed_index + 1; second_seed_observed_index < query_star_count; ++second_seed_observed_index) {
            ...
            for (uint8_t seed_candidate_index = 0; seed_candidate_index < seed_candidate_count; ++seed_candidate_index) {
                ...
                /* Each grow attempt gets a fresh budget so early pairs cannot starve later ones. */
                ...
                if (state.best.score >= PYRAMID_EARLY_EXIT_SCORE) {
                    goto pyramid_done;
                }
    ...
    *result = state.best;
    {
        ...
        result->verify_us = state.verify_us;
        result->db_us = total_us > state.verify_us ? total_us - state.verify_us : 0;
    }
